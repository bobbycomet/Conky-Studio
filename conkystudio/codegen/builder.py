...
def build_project(project: Project, output_dir: str, source_search_dirs: list | None = None) -> BuildResult:
    ...
    used_custom_ids = {n.id for n in _used_source_nodes(project) if n.type == "source.custom_script"}
    for n in custom_nodes:
        ...
        if not src_path or not os.path.isfile(src_path):
            ...
            # Still write a stub so the theme folder builds cleanly and the
            # missing-file warning is the only thing the user has to act on.
