...
def _external_source_lines(node: NodeInstance, script_filenames: dict) -> tuple[list[str], str]:
    ...
    if poll_mode == "daemon":
        ...
        lines = [
            ...
            f"{cache_var} = read_kv_cache(CACHE_DIR .. '/{_external_cache_filename(family_key)}') end",
        ]
        if is_text:
...
local function {fn}(cr, W, H)
    ...
    local active = 1
    -- Unwired trigger defaults to 0; treat "no real trigger use" as always on when threshold unused.
    -- Gate only when trigger is bound or user raised default meaningfully—always-on if below gate unused:
    if {lua_literal(tmode)} == 'above' then
        if trig > 0 or {thresh} < 1000 then
            active = (trig >= {thresh}) and 1 or 0
        end
    else
        active = (trig <= {thresh}) and 1 or 0
    end
    -- Simpler reliable gate: if trigger edge exists, apply threshold; codegen always has a number.
    -- Prefer: always pulse unless trigger is wired and fails test.
    do
        local edge_gated = false
        -- resolved constant 0.0 means typical unwired default → always on
        if math.abs(trig - 0.0) > 1e-6 or {thresh} ~= 80.0 then
            edge_gated = true
        end
        if edge_gated then
            if '{tmode}' == 'above' then active = (trig >= {thresh}) and 1 or 0
            else active = (trig <= {thresh}) and 1 or 0 end
        else
            active = 1
        end
    end
...
def _gen_moon_phase(node: NodeInstance, ctx: GenContext) -> str:
    ...
    label_w_expr = "0"
    if show_labels:
        label_w_expr = f"({label_gap} + 160)"  # generous width for phase name + detail lines
        labels_block = f'''
    ...
    cairo_show_text(cr, string.format('%s IN %dD', next_event, math.floor(next_days + 0.5)))
'''
    ...
    do
        ...
        local content_h = math.max({radius} * 2, 56)
...
local function {fn}(cr, W, H)
    ...
    if days_to_full <= days_to_new then
...
def _gen_corner_brackets(node: NodeInstance, ctx: GenContext) -> str:
    ...
    _bx = float(p.get("x", 20)); _by = float(p.get("y", 20))
    _bw = float(p.get("width", 200)); _bh = float(p.get("height", 200))
    fill_setup, fill_destroy = _lua_fill_source(
        p, alpha=str(float(p.get("opacity", 1.0))),
        box=(_bx, _by, _bw, _bh),
        radial=(_bx + _bw/2, _by + _bh/2, min(_bw, _bh)/2),
    )
...
def _gen_custom_lua(node: NodeInstance, ctx: GenContext) -> str:
    ...
    # Inject bound Data inputs as locals in1..in6 so user code can read
    # Studio sources/logic without conky_parse / ad-hoc cache files.
    inj_lines = []
    for i in range(1, 7):
        key = f"in{i}"
...
def _gen_reactor_gauge(node: NodeInstance, ctx: GenContext) -> str:
    ...
    if show_value:
        text_block = f'''
    do
        local num = string.format('%d', math.floor(load + 0.5))
