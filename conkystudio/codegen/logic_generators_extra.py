...
Helpers are local so this module never imports lua_gen (avoids a circular
import while lua_gen is still executing its module body).
"""
...
def _lua_literal(value) -> str:
    ...
    if isinstance(value, (int, float)):
        return repr(value)
    return _lua_string_literal(str(value))
...
def register(logic_generator):
    ...
    def _logic_smooth(node, ctx):
        ...
        key = _lua_string_literal(node.id)
        if init:
            return (
                f"(function() "
    ...
    def _logic_string_join(node, ctx):
        ...
        sep = _lua_string_literal(str(node.props.get("separator", " ") or " "))
    ...
    def _logic_enum_map(node, ctx):
        ...
        while len(vals) < len(keys):
            vals.append(0.0)
        pairs = ", ".join(
            f"[{_lua_string_literal(k)}] = {_lua_literal(vals[i])}"
        ...
        return (
            ...
            f"return {_lua_literal(default)}; end)()"
        )
