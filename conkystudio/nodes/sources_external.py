...
_POLL_MODE = lambda default_interval, execi_default=2, default_mode="execi": [  # noqa: E731 -- tiny local factory, not worth a def
    PropertySpec(key="poll_mode", label="Polling mode", kind=ENUM, default=default_mode,
...
# Sensors-subcategory sources (cpu/gpu/disk temp, fan RPM, GPU util/VRAM)
# default to background-daemon polling: these are the nodes most likely to
# sit in a high-FPS HUD and most likely to be the one thing that stutters
# a draw frame if their execi cache expires mid-redraw. execi is still the
# right default everywhere else in this file (Public IP/Weather/Now
# Playing/Custom Script are either infrequent or already default to daemon
# individually for their own reasons above).
_SENSOR_POLL_MODE = lambda default_interval: _POLL_MODE(default_interval, default_mode="daemon")  # noqa: E731
...
register(NodeSpec(
    ...
    scripted=True,  # not a native ${...} source -- see NodeSpec.scripted
    properties=[
