...
class NodeSpec:
    ...
    # True for a "source" node whose value comes from running a script
    # (execi call or daemon cache read) rather than a native ${...} conky
    # variable, when script_family is None (i.e. NOT one of the shared
    # cpu_sensors/gpu_stats/... families). source.custom_script and every
    # source-category plugin (see plugins/loader.py) set this; codegen's
    # refresh_sources() dispatch (codegen/lua_gen.py) uses it to tell a
    # "real native source, no script" node apart from a "one-script-per-
    # node-instance" one without hard-coding node type strings.
    scripted: bool = False
...
def register(spec: NodeSpec, *, replace: bool = False) -> NodeSpec:
    ...
    (used when re-installing a plugin after uninstall).

    Re-registration of the same type is a no-op when the new spec is the same
    object, or when label + category + output_kind match (property-list length
    may differ during a gradient/prop upgrade). The first registration wins so a
    packaging mix-up that loads visuals.py twice (or a mis-shipped lua_gen.py
    that re-registers NodeSpecs) cannot hard-crash the app.

    A *different* label/category for the same type still raises unless replace=True.
    """
    existing = _REGISTRY.get(spec.type)
    if existing is not None and not replace:
        if existing is spec:
            return existing
        # Same logical node type (label/category/kind) → keep the first registration.
        # Property counts often diverge briefly when gradient/blend props are added
        # in one module but not another; raising here was the AppImage startup crash.
        if (
            existing.label == spec.label
            and existing.category == spec.category
            and existing.output_kind == spec.output_kind
        ):
            if len(existing.properties) != len(spec.properties):
                print(
                    f"[conky-studio] note: '{spec.type}' registered twice with "
                    f"{len(existing.properties)} vs {len(spec.properties)} properties "
                    f"— keeping the first. Check that codegen/lua_gen.py does not "
                    f"contain NodeSpec register() calls (those belong in nodes/visuals*.py)."
                )
            return existing
        raise ValueError(
