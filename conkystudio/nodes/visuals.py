...
from conkystudio.nodes.registry import (
    ...
    KIND_PERCENT, KIND_CELSIUS, KIND_NUMBER, KIND_TEXT, KIND_CATEGORY, NUMERIC_KINDS,
)
...
register(NodeSpec(
    ...
    properties=[
        PropertySpec(key="value", label="Value", kind=STRING, default="Label", bindable=True,
                     accepts=(KIND_TEXT, KIND_CATEGORY, KIND_NUMBER, KIND_PERCENT, KIND_CELSIUS),
                     help="Plain text if left unwired, or drag a source node's output onto this."),
...
register(NodeSpec(
    ...
    properties=[
        PropertySpec(key="value", label="Value", kind=STRING, default="Line one\nLine two", bindable=True,
                     accepts=(KIND_TEXT,), help="Newline-separated. Usually a Custom Script source."),
        PropertySpec(key="x", label="X", kind=INT, default=20, minimum=-4000, maximum=4000, group="Position"),
        PropertySpec(key="y", label="Y", kind=INT, default=20, minimum=-4000, maximum=4000, group="Position"),
...
register(NodeSpec(
    ...
    subcategory="Effects",
    description="Astronomical moon phase disc with phase name, illumination %, and days to next "
...
# --------------------------------------------------------------- Custom Lua
# Bindable in1..in6 let you wire Studio sources / logic (sensors, math, …)
# into imported or hand-written Cairo. At draw time each bound input is a
# local in the generated function; replace conky_parse / file reads with them.
_CUSTOM_LUA_INPUTS = [
    PropertySpec(
        key=f"in{i}", label=f"Input {i}", kind=FLOAT, default=0.0,
        bindable=True,
        accepts=(KIND_PERCENT, KIND_CELSIUS, KIND_NUMBER, KIND_TEXT, KIND_CATEGORY),
        group="Data inputs",
        help="Wire a source or logic node here. Available in Lua as local in%d (number or string)."
             % i,
    )
    for i in range(1, 7)
]
register(NodeSpec(
    ...
    description="Raw Cairo drawing code inside this node's draw call (cr, W, H already in scope). "
                "Wire sources or logic into Input 1–6 and read them as locals in1..in6 — same "
                "data → logic → visual flow as native nodes. The legacy importer lands unmapped "
                "drawing here; use Offset X/Y to shift the block without editing the Lua.",
    properties=[
        ...
        *_CUSTOM_LUA_INPUTS,
        PropertySpec(key="code", label="Lua code", kind=CODE,
                     default="-- cr, W, H are in scope\n"
                             "-- Wired sources appear as locals in1..in6 (nil/0 if unbound)\n"
                             "-- Example: cairo_set_source_rgba(cr, 1, 1, 1, 1)\n"
                             "--          -- use tonumber(in1) or in1 for a bound sensor\n",
                     help="Runs inside local function(cr, W, H). Bound Data inputs are injected "
                          "as locals in1..in6 before your code. Framework helpers (clamp, lerp, "
