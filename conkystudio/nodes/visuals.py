...
# Full gradient fill (solid | linear | radial) + extend mode, shared with
# visuals_more / visuals_extra / gradient_integration so every fill-capable
# node stays in sync. Generators read these via fill_source_lua().
# Also expose blend_mode (additive/screen/…) for glow-style compositing.
_GRADIENT_FILL = gradient_property_specs()
_BLEND = [blend_property_spec()]
...
register(NodeSpec(
    ...
    properties=[
        ...
        PropertySpec(key="variant", label="Render variant", kind=ENUM, default="v2",
                     choices=["v1", "v2"], choice_labels=["Variant 1 (Classic)", "Variant 2 (Gradient-aware)"],
                     group="Style",
                     help="Two independent implementations of this dial ship side by side. "
                          "Variant 1 is the original dashed-ring dial. Variant 2 adds gradient "
                          "and blend-mode support (Style group above) and refined text layout. "
                          "Switching this only changes which draw code runs -- other properties "
                          "carry over."),
        *_GRADIENT_FILL, *_BLEND,
        *_SCALE,
    ],
...
register(NodeSpec(
    ...
    description="Raw Cairo drawing code inside this node's draw call (cr, W, H already in scope). "
                "Wire sources or logic into Input 1–12 (locals in1..in12). Every node also gets "
                "NODE_ID, a private persistent NS table, and its own props as a PROPS table -- see "
                "the Plugin SDK group below. Legacy import auto-wires common vitals into these "
                "inputs and patches the Lua so bound values win over internal conky_parse/cache "
                "reads. Offset X/Y shifts the block without editing Lua.",
    properties=[
        ...
        PropertySpec(key="no_translate", label="Skip auto-translate", kind=BOOL, default=False, group="Position",
                     help="Off (default): the code sees a canvas already translated by Offset X/Y, "
                          "matching every prior version of this node. On: skips that translate, so "
                          "the code gets the raw canvas and is fully responsible for its own "
                          "positioning -- useful for code that draws several independently placed "
                          "elements instead of one thing anchored at a single x/y."),
        *_CUSTOM_LUA_INPUTS,
        PropertySpec(key="code", label="Lua code", kind=CODE,
                     default="-- cr, W, H are in scope\n"
                             ...
                             "-- NODE_ID / NS / PROPS are also in scope -- see the Plugin SDK group\n",
                     help="Runs inside local function(cr, W, H). Bound Data inputs are injected "
                          ...
                          "rounded_rect, load_image_cached, wall_clock, ...) are available, plus "
                          "the Plugin SDK: NODE_ID (this node's id), NS (a private table persisted "
                          "across frames -- prefer NS.foo over STATE[key] so copy-pasted nodes never "
                          "collide), PROPS (this node's props as a Lua table), and studio_define / "
                          "studio_use for sharing code with other Custom Lua nodes (see Run mode)."),
        PropertySpec(key="run_mode", label="Run mode", kind=ENUM, default="draw",
                     choices=["draw", "module"],
                     choice_labels=["Draw (runs every frame)", "Module (runs once, defines shared code)"],
                     group="Plugin SDK",
                     help="Draw (default): unchanged behaviour -- compiles to a per-frame draw "
                          "function, wrapped so an error here only skips this node for that frame "
                          "instead of the whole HUD. Module: this node draws nothing and instead "
                          "runs once when the theme loads, meant for a single studio_define(name, "
                          "{...}) call that other Draw-mode nodes then pull in via studio_use(name) "
                          "-- lets a plugin ship one shared helper library instead of the same code "
                          "pasted into every node. A Module node's canvas visibility is irrelevant "
                          "since it never draws; toggling it off to declutter the canvas view is "
                          "fine and does not disable it."),
        PropertySpec(key="load_order", label="Module load order", kind=FLOAT, default=0.0,
                     minimum=-1000, maximum=1000, step=1, group="Plugin SDK",
                     help="Module mode only. Lower values load first. Only matters if one Module "
                          "node's code calls studio_use() on a name defined by another Module node "
                          "-- give the dependency the lower number so it's defined first. Ignored "
                          "in Draw mode."),
        *_SCALE,
    ],
))
