...
Providers typically expose:
  ...
  (www.opendesktop.org/ocs/v1/ is deprecated and returns HTTP 410)
...
DEFAULT_PROVIDERS: dict[str, str] = {
    ...
    # www.opendesktop.org/ocs/v1/ returns HTTP 410 (deprecated); the live
    # endpoint is api.opendesktop.org (same backend family as Pling).
...
class OcsClient:
    ...
    def categories(self) -> list[OcsCategory]:
        ...
        # _data_list validates statuscode (nested meta or flat top-level).
    ...
    def _data_list(self, data: dict, singular: str) -> list[dict]:
        ...
        OCS wraps payloads in meta/data. Content lists may be:
          ...
          data[singular]
          data          (list of content objects — Pling / api.opendesktop flat JSON)
        """
        root = data.get("ocs", data)
        # Classic nested meta, or flat Pling-style statuscode at the top level.
        meta = root.get("meta") or data.get("meta")
        if not isinstance(meta, dict) or not meta:
            meta = {
                "statuscode": root.get("statuscode") or data.get("statuscode"),
                "status": root.get("status") or data.get("status"),
                "message": root.get("message") or data.get("message") or "",
            }
        if isinstance(meta, dict):
            self._check_meta(meta)

        payload = root.get("data") if "data" in root else data.get("data")
        if payload is None:
            payload = root

        # Flat JSON from Pling / api.opendesktop: "data" is already a list of items.
