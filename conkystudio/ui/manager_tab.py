...
class _DropArea(QFrame):
    def __init__(self, on_files, parent=None):
        ...
        label = QLabel("Drop a .zip or .tar.gz theme here to install it, or click to browse")
    ...
    def mousePressEvent(self, event):
        paths, _ = QFileDialog.getOpenFileNames(self, "Choose theme archive(s)", "", "Theme archives (*.zip *.tar.gz *.tgz)")
        if paths:
...
class ManagerTab(QWidget):
    ...
    def _start_selected(self):
        t = self._selected_theme()
        if t:
            self.process_manager.start(t.path)
