...
from conkystudio.codegen.theme_common import pin_sensor_poll_mode
...
CATEGORY_STYLES = {
    "Minimal": {
        "accent": "#7fd9c4", "accent2": "#5a8fd6", "track": "#2a2f38", "panel": "#14181e",
        "flourish": False, "font": "Sans",
        "text_dim": "#9aa2ad", "warn": "#ff6b6b", "layout": "minimal",
    },
    "Gaming": {
        "accent": "#9d7bf0", "accent2": "#ff4d9e", "track": "#241f38", "panel": "#120e1c",
        "flourish": True, "font": "Sans",
        "text_dim": "#b8a0e0", "warn": "#ff4d6d", "layout": "gaming",
    },
    "RPG": {
        "accent": "#d9b23a", "accent2": "#8b5a2b", "track": "#3a2f1c", "panel": "#1a1510",
        "flourish": True, "font": "Sans",
        "text_dim": "#d4c4a0", "warn": "#c0463f", "layout": "rpg",
    },
    "Sci-Fi": {
        "accent": "#2ee6d6", "accent2": "#3f7fe0", "track": "#0a3a40", "panel": "#061418",
        "flourish": True, "font": "Share Tech Mono",
        "text_dim": "#5fd8ce", "warn": "#ff3b3b", "layout": "scifi",
    },
    "Cyberpunk": {
        "accent": "#f43fa0", "accent2": "#00d9ff", "track": "#2a1030", "panel": "#120810",
        "flourish": True, "font": "Sans",
        "text_dim": "#f0a0c8", "warn": "#ffe14d", "layout": "cyberpunk",
    },
    "Terminal": {
        "accent": "#4fd67f", "accent2": "#2c8f56", "track": "#0f1a12", "panel": "#080c09",
        "flourish": False, "font": "Share Tech Mono",
        "text_dim": "#7dba94", "warn": "#e0b04a", "layout": "terminal",
    },
    "Fantasy": {
        "accent": "#c2ac82", "accent2": "#7c5a3a", "track": "#2f2a1c", "panel": "#16140f",
        "flourish": True, "font": "Sans",
        "text_dim": "#d4c8b0", "warn": "#a5482c", "layout": "fantasy",
    },
    "Arkhamos": {
        "accent": "#3fd6ff", "accent2": "#1a4fb8", "track": "#0a2a35", "panel": "#050e12",
        "flourish": True, "font": "Share Tech Mono",
        "warn": "#ff3b3b", "text_dim": "#4fb8d6", "layout": "arkhamos",
    },
}
CATEGORY_ORDER = ["Minimal", "Gaming", "RPG", "Sci-Fi", "Cyberpunk", "Terminal", "Fantasy", "Arkhamos"]
...
# Extra chrome / behaviour toggles (not data panels). Ordered roughly by
# tier: the first five existed before and stay Full-tier defaults; the
# last four are new and default on only at Showcase (see
# theme_wizard_patch.DEFAULT_OPTIONS_BY_TIER) so Simple/Full stay the
# "solid HUD" look and Showcase is the one that visibly goes further.
OPTION_ORDER = [
    ...
    ("glow", "Glow accents"),
    ("gradient_fills", "Gradient fills"),
    ("secondary_gauge", "Secondary trend readouts"),
    ("signature_effect", "Signature flourish"),
    ("footer_ticker", "Footer ticker line"),
]

DEFAULT_PANELS = ["Weather", "CPU", "GPU", "RAM", "Clock", "Network", "System"]
DEFAULT_OPTIONS = {
    "chrome": True, "leds": True, "graphs": True, "brackets": True, "glow": True,
    "gradient_fills": False, "secondary_gauge": False, "signature_effect": False, "footer_ticker": False,
}
...
def _grad(style: dict, options: dict, *, angle: float = 90.0, mode: str = "linear") -> dict:
    """fill_mode/color_end/gradient_angle props for any node carrying the
    shared gradient-fill prop set (visual.bar, visual.arc_gauge,
    visual.reactor_gauge, visual.history_graph, ...) -- empty dict (no-op,
    node keeps its flat colour) unless the user turned Gradient fills on."""
    if not options.get("gradient_fills"):
        return {}
    return {
        "fill_mode": mode,
        "color_end": style.get("accent2", style["accent"]),
        "gradient_angle": angle,
    }


def _trend_sparkline(p: Project, source_type: str, x: float, y: float, style: dict,
                      w: int = 90, h: int = 20, source_props: dict | None = None):
    """Small trend line under a vital -- the 'Secondary trend readouts'
    option's payload. Uses the same source TYPE the vital above it reads
    (a second node instance; cheap, and keeps each vital self-contained
    rather than threading shared source ids through every caller)."""
    if not _has_node("visual.sparkline"):
        return
    src = p.add_node(NodeInstance(id=new_id("n"), type=source_type, x=x - 260, y=y, props=dict(source_props or {})))
    spark = p.add_node(NodeInstance(id=new_id("n"), type="visual.sparkline", z=p.next_z(), x=x, y=y, props={
        "x": int(x), "y": int(y), "width": w, "height": h,
        "color": style.get("accent2", style["accent"]), "fill": True,
    }))
    p.add_edge(src.id, spark.id, "value")
...
def _header(p: Project, title: str, subtitle: str, style: dict, width: int, options: dict | None = None):
    ...
    if options.get("footer_ticker") and _has_node("source.uptime"):
        up = p.add_node(NodeInstance(id=new_id("n"), type="source.uptime", x=-200, y=60, props={"format": "short"}))
        up_lbl = p.add_node(NodeInstance(id=new_id("n"), type="visual.text", z=p.next_z(), x=width - 200, y=60, props={
            "value": "", "prefix": "up ", "x": width - 48, "y": 60, "align": "right",
            "font_family": font, "font_size": 10, "color": dim,
        }))
...
def _add_arc_vital(p: Project, source_type: str, label: str, cx: float, cy: float, style: dict,
    ...
    if options.get("secondary_gauge"):
        _trend_sparkline(p, source_type, cx - 34, cy + radius + 22, style, w=radius * 2 - 12, h=12)
    return src
...
def _build_minimal_layout(p: Project, style: dict, panels: list, width: int, height: int,
    ...
    _header(p, "SYSTEM", "minimal monitor", style, width, options)
    ...
    if options.get("signature_effect") and _has_node("visual.orbit_field"):
        cpu = p.add_node(NodeInstance(id=new_id("n"), type="source.cpu_percent", x=-200, y=height - 60))
        orbit = p.add_node(NodeInstance(id=new_id("n"), type="visual.orbit_field", z=0, x=width - 100, y=height - 100, props={
            "cx": width - 60, "cy": height - 60, "radius": 34, "dot_count": 8,
            "rings": 1, "color": style.get("accent2", style["accent"]), "opacity": 0.4, "speed_dps": 14,
        }))
...
def _build_gaming_layout(p: Project, style: dict, panels: list, width: int, height: int,
    ...
    _header(p, "PERFORMANCE", "gaming overlay", style, width, options)
    ...
    if options.get("signature_effect") and _has_node("visual.reactor_gauge") and gauges:
...
def _build_rpg_layout(p: Project, style: dict, panels: list, width: int, height: int,
    ...
    _header(p, "STATUS", "character sheet", style, width, options)
    ...
    if options.get("signature_effect") and _has_node("visual.reactor_gauge"):
        src = p.add_node(NodeInstance(id=new_id("n"), type="source.cpu_percent", x=-200, y=height - 220))
        rg = p.add_node(NodeInstance(id=new_id("n"), type="visual.reactor_gauge", z=p.next_z(), x=width - 220, y=height - 220, props={
            "cx": width - 140, "cy": height - 140, "radius": 66,
            "color": style["accent"], "dim_color": style["track"],
            "accent_color": style.get("warn", "#c0463f"),
            "label": "POWER LEVEL", "font_family": style["font"],
            **_grad(style, options, angle=45),
        }))
        p.add_edge(src.id, rg.id, "value")
...
def _build_scifi_layout(p: Project, style: dict, panels: list, width: int, height: int,
    ...
    _header(p, "SYSTEM INTERFACE", "sci-fi telemetry", style, width, options)
    ...
    if options.get("signature_effect") and _has_node("visual.reactor_gauge"):
        ...
        p.add_edge(src.id, rg.id, "value")
    if options.get("signature_effect") and _has_node("visual.radar_sweep"):
...
def _build_cyberpunk_layout(p: Project, style: dict, panels: list, width: int, height: int,
    ...
    _header(p, "NIGHT CITY", "cyberdeck telemetry", style, width, options)
    ...
    if options.get("signature_effect") and _has_node("visual.spiral"):
...
def _build_terminal_layout(p: Project, style: dict, panels: list, width: int, height: int,
    ...
    _header(p, "$ top", "terminal session", style, width, options)
...
def _build_fantasy_layout(p: Project, style: dict, panels: list, width: int, height: int,
    ...
    _header(p, "GRIMOIRE", "arcane monitor", style, width, options)
...
def _arkhamos_vital_row(
    ...
    *,
    ...
    """One Arkhamos-style vital: LABEL + segmented bar + optional temp text.
...
def _build_arkhamos_layout(p: Project, style: dict, panels: list, width: int, height: int,
                         options: dict | None = None) -> None:
    """Dense vigilante-console layout using Studio sources / visuals / logic.
    ...
    updates. The one decorative-only piece (the signal hologram) stays as
    optional Custom Lua, gated behind the Signature flourish option, so
    the graph remains the source of truth for CPU / GPU / RAM / network /
    weather.
    """
    options = _opts(options)
    accent = style["accent"]
    ...
    _header(p, "ARKHAMOS", "SENTINEL CORE  v1.0", style, width, options)

    # ---- left column: vitals (vitals column) ----
    vx, vy = 40, 100
    if options.get("chrome"):
    ...
    if "CPU" in panels:
        cpu_u, cpu_t = _arkhamos_vital_row(
    ...
    if "GPU" in panels and _has_node("source.gpu_util"):
        gpu_u, gpu_t = _arkhamos_vital_row(
    ...
    if "Disk" in panels:
        disk_u, disk_t = _arkhamos_vital_row(
    ...
    # Image slot for a signal/logo asset (user drops it on the node after create)
    ...
    # LED + glow already signal warn. Optional Custom Lua can tint the hologram.
    ...
    p.add_node(NodeInstance(id=new_id("n"), type="visual.text", z=p.next_z(), x=cx - 120, y=cy + 178, props={
        "value": "Set Image path on the icon node above  ·  glow follows threat gate", "x": cx, "y": cy + 178,
    ...
    # signal-hologram placeholder (user can paste a full custom draw routine).
    # Gated behind Signature flourish so Simple/Full stay a clean vitals
    # console and this animated centerpiece is what Showcase adds.
    if options.get("signature_effect") and _has_node("visual.custom_lua"):
        bat_lua = p.add_node(NodeInstance(
            id=new_id("n"), type="visual.custom_lua", z=p.next_z(),
            x=cx - 40, y=cy - 200,
            label="Signal hologram (Custom Lua)",
            props={
                "x": 0, "y": 0,
                "code": (
                    ...
                    "-- Replace this block with your own full silhouette/tint routine\n"
                    "-- after dropping an image on the Image node above.\n"
                ),
            },
        ))
        if threat_src:
    ...
    # ---- bottom: network + thermal history (dual graphs) ----
    hist_y = height - 150
    if "Network" in panels:
        _net_graph(p, 40, hist_y, 380, 90, style, "NETWORK THROUGHPUT", options)
    ...
    if options.get("graphs") and ("CPU" in panels or "GPU" in panels) and _has_node("visual.multi_line_graph"):
        ...
        p.add_node(NodeInstance(id=new_id("n"), type="visual.text", z=p.next_z(), x=width - 420, y=hist_y - 16, props={
            "value": "THERMAL TREND", "x": width - 420, "y": hist_y - 8,
...
LAYOUT_BUILDERS = {
    ...
    "arkhamos": _build_arkhamos_layout,
}
...
def build_wizard_project(
    ...
    # Teaching-tool default: see pin_sensor_poll_mode()'s docstring.
    pin_sensor_poll_mode(p)
    return p
...
class ThemeWizardDialog(QDialog):
    def __init__(self, parent=None, *, startup_mode: bool = False):
        ...
        self.cat_box = QGroupBox("Category")
        cat_layout = QGridLayout()
        self.cat_box.setLayout(cat_layout)
        ...
        # The tour is scoped to "you've picked a theme" -- it stays hidden
        # until the user actually clicks a category radio (buttonClicked
        # doesn't fire for the setChecked(True) above), then appears next
        # to the button row below.
        self.category_group.buttonClicked.connect(self._on_category_chosen)
        ...
        self.panel_box = QGroupBox("Panels")
        panel_layout = QGridLayout()
        self.panel_box.setLayout(panel_layout)
        self.panel_checks = {}
        ...
        layout.addWidget(self.panel_box)

        self.opt_box = QGroupBox("Extras")
        ...
        self.tier_box = QGroupBox("Complexity")
        tier_layout = QHBoxLayout()
        self.tier_box.setLayout(tier_layout)
        ...
        layout.addWidget(self.tier_box)
        ...
        note = QLabel(
            ...
            "gradient fills, secondary trend readouts, each theme's signature flourish, and a footer ticker. "
            ...
            "Attach images on Image nodes after create."
        )
        ...
        self.tour_btn.setVisible(False)
        self.tour_btn.clicked.connect(self._on_tour)
    ...
    def _on_category_chosen(self, radio):
        """A theme has actually been picked (not just the startup default) --
        reveal the tour button. Kept separate from __init__ so the button
        doesn't clutter the dialog before the user has engaged with it."""
        self.tour_btn.setVisible(True)

    def start_tour(self):
        """Public entry point: launches the tour directly, bypassing the
        "click a category first" gate the in-dialog button uses. Lets an
        external caller (MainWindow's Help menu) open the wizard already
        mid-tour instead of making the person click the button themselves."""
        from conkystudio.ui.studio.theme_wizard_tour import ThemeWizardTour
        self._tour = ThemeWizardTour(self)
        self._tour.start()

    def prepare_for_guided_tour(self):
        """Pre-select Minimal + Showcase for the full-app guided tour.

        Also reveals the in-dialog tour button and applies Showcase panel /
        extras defaults so Create yields a dense learning graph.
        """
        for btn in self.category_group.buttons():
            if btn.text() == "Minimal":
                btn.setChecked(True)
                self._on_category_chosen(btn)
                break
        for btn in self.tier_group.buttons():
            if btn.text() == "Showcase":
                btn.setChecked(True)
                self._on_tier_changed(btn)
                break
        self.tour_btn.setVisible(True)
    ...
    def _on_create(self):
        ...
        # Folder / lock-file safe name: spaces break install paths and start.sh
        # locks. Use a hyphen so "Minimal HUD" becomes "Minimal-HUD".
        safe_name = f"{category}-HUD".replace(" ", "-")
        self.result_project = build_wizard_project(
            safe_name, category, resolution, panels, options,
