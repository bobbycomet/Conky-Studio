    ...
    def _make_editor(self, node, pspec) -> QWidget:
        ...
        if kind == registry.PATH:
            ...
            def do_browse():
                ...
                # script_path uses the same PATH editor — broaden the filter.
                is_script = prop_key == "script_path"
                if is_script:
                    caption = "Choose script file"
                    filt = "Scripts (*.sh *.bash *.py *.lua);;All files (*)"
                    asset_kind = "scripts"
                else:
                    caption = "Choose image file"
                    filt = "Images (*.png *.svg *.jpg *.jpeg *.webp);;All files (*)"
                    asset_kind = "images"
                ...
                if not path:
                    return
                # When the project is (or will be) a .cstudio package, copy
                # the file into assets/ and store a package-relative path so
                # the HUD stays portable. Legacy bare-JSON sessions still
                # keep the absolute path until the user Saves as .cstudio.
                store_path = path
                try:
                    if self.project is not None and (
                        self.project.package_root
                        or (self.project.source_path or "").lower().endswith(".cstudio")
                    ):
                        store_path = self.project.import_asset_file(path, kind=asset_kind)
                except OSError:
                    store_path = path
                edit.blockSignals(True)
                edit.setText(store_path)
                edit.blockSignals(False)
                _write_path(store_path)
