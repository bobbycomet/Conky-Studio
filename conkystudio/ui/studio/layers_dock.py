...
class LayersDock(QWidget):
    ...
    def _on_order_changed(self, ordered_ids: list):
        ...
        selected_id = selected[0].data(_NODE_ID_ROLE) if selected else None
        ...
        if selected_id is not None:
            self.select_node(selected_id)   # refresh() rebuilds fresh items, so the drop target loses its highlight otherwise
