...
# Slightly roomier than the original 176×26 so multi-word titles and a
# short description fit without clipping. Height for description nodes is
# measured after layout (see NodeItem), not fixed to one ROW_H.
NODE_WIDTH = 200
HEADER_H = 28
ROW_H = 22
SOCKET_R = 5
# Character budget for the body blurb on source / non-bindable nodes.
# Wider nodes can show a bit more before the ellipsis; height still grows
# with the wrapped text so nothing runs off the bottom edge.
_DESC_CHARS = 48
...
class NodeItem(QGraphicsRectItem):
    def __init__(self, node_id: str, node_type: str, label: str = "", parent=None):
        ...
        # Provisional height; description nodes may grow after text layout.
        ...
        title.setFont(f)
        title.setPos(8, 4)
        # Cap title to the header width. Prefer a single visual line: if the
        # label is longer than the node, Qt still clips to the header strip
        # rather than pushing body content down (height is measured from
        # header + body, not title wrap).
        ...
        # Grow the rect when a description wraps to more than one line so
        # text never runs off the bottom of the node.
        if desc_item is not None:
            body_h = max(ROW_H + 10, int(desc_item.boundingRect().height()) + 12)
            height = HEADER_H + body_h
            self.setRect(0, 0, NODE_WIDTH, height)

        if self.output_socket is not None:
            self.output_socket.setPos(NODE_WIDTH, HEADER_H + (height - HEADER_H) / 2)

    def _make_desc(self, y: float) -> QGraphicsTextItem:
        desc = QGraphicsTextItem(self._short(self.spec.description, _DESC_CHARS), self)
