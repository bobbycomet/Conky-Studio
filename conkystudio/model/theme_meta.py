...
class ThemeMeta:
    ...
    # Optional Manager pin: "auto" | "primary" | concrete output name (DP-1, HDMI-A-1, …).
    # When set to a concrete output, ThemeProcessManager injects xinerama_head at start.
    monitor: str = "auto"
    ...
    def to_dict(self) -> dict:
        ...
        if d["sha256"] is None:
            d.pop("sha256")
        # Omit default monitor so older themes stay tidy
        if d.get("monitor") in (None, "", "auto"):
            d.pop("monitor", None)
        return d
    ...
    def from_dict(d: dict) -> "ThemeMeta":
        return ThemeMeta(
            ...
            sha256=d.get("sha256"),
            monitor=str(d.get("monitor") or "auto"),
        )
