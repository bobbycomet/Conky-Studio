...
def _find_theme_root(extracted_dir: str) -> str:
    """Archives are sometimes a theme's files directly, sometimes wrapped
    in one extra folder (GitHub's 'Download ZIP' does this) -- detect
    the wrapped case by checking for a single subdirectory that itself
    contains theme.json/conky.conf/start.sh."""
    ...
    if len(entries) == 1:
        ...
        if os.path.isdir(candidate):
            ...
            if any(os.path.isfile(os.path.join(candidate, m)) for m in markers):
                return candidate
...
def install_theme_archive(archive_path: str, install_root: str = DEFAULT_INSTALL_ROOT) -> InstallResult:
    ...
    if os.path.isfile(meta_path):
        ...
        theme_name = os.path.splitext(os.path.basename(archive_path))[0]
        if theme_name.endswith(".tar"):
            theme_name = theme_name[: -len(".tar")]
    ...
    return InstallResult(
        ...
        message=f"Installed '{theme_name}'" + (f" (+{len(installed_fonts)} font(s))" if installed_fonts else ""),
    )
