...
def _is_self_caching_script(src: str) -> bool:
    """True for sensors.sh / weather.sh style scripts that write their own
    CACHE_FILE (read later by Custom Lua) instead of printing a single
    value to stdout."""
    if not src:
        return False
    return bool(re.search(
        r'CACHE_FILE\s*=|sensors\.cache|weather\.cache|\.runtime-cache',
        src,
    ))
...
def _patch_script_cache_dir(script_path: str, scripts_out_dir: str) -> str:
    ...
    CACHE_DIR resolves to in render.lua). The patched file is placed in
    scripts_out_dir (_imported_scripts/); builder later copies it into
    the built theme's scripts/, so dirname/.. correctly resolves to the
    theme root. Returns the path to use as script_path (patched copy or
    original)."""
    try:
    ...
    # Match common hard-coded forms:
    #   CACHE_DIR="${HOME}/.cache/conky-batman"
    #   CACHE_DIR="$HOME/.cache/foo"
    #   CACHE_DIR=/home/user/.cache/foo
    #   CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/conky-batman"
    new_line = (
        'CACHE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/.runtime-cache"'
    )
    patched = re.sub(
        r'''CACHE_DIR\s*=\s*["'](?:\$\{?HOME\}?|\$\{?XDG_CACHE_HOME[^"']*|/(?:home|Users)/[^/"']+)/\.cache/[^"']*["']''',
        new_line,
        src,
    )
    # Unquoted variant: CACHE_DIR=$HOME/.cache/foo
    if patched == src:
        patched = re.sub(
            r'''CACHE_DIR\s*=\s*(?:\$\{?HOME\}?|\$\{XDG_CACHE_HOME:-?\$HOME\}?)/\.cache/[^\s;&#]+''',
            new_line,
            src,
        )
    # Absolute /home/... or /Users/... without $HOME
    if patched == src:
        patched = re.sub(
            r'''CACHE_DIR\s*=\s*["']?/(?:home|Users)/[^/"'\s]+/\.cache/[^"'\s]+["']?''',
            new_line,
            src,
        )
    if patched == src:
...
def _find_companion_conf(script_path: str) -> str:
    """sensors.sh often ships with sensors.conf in the same folder; same
    for weather.sh / weather.conf. Return the path if present."""
    if not script_path:
        return ""
    d = os.path.dirname(script_path)
    base = os.path.basename(script_path)
    candidates = [
        os.path.join(d, base.replace(".sh", ".conf").replace(".bash", ".conf")),
        os.path.join(d, "sensors.conf"),
        os.path.join(d, "weather.conf"),
    ]
    for c in candidates:
        if os.path.isfile(c):
            return c
...
BOILERPLATE_PATTERNS = [
    ...
    # "if updates < 5 then return end" when updates was just assigned
    re.compile(
        r"local\s+updates\s*=\s*tonumber\([^)]+\)\s*or\s*0\s*\n"
        r"\s*if\s+updates\s*<\s*\d+\s+then\s+return\s+end\s*\n?"
    ),
]
...
def _extract_draw_hook_body(lua_text: str, hook_name: str | None) -> str:
    """Pull helpers (preamble) + main draw-hook body into one chunk for
    Custom Lua. Prefer a greedy match to the last `end` of the file when
    the hook is the final function (Batman/Skyrim style) so nested
    `end`s inside pcall/closures are not truncated."""
    candidates = []
    if hook_name:
        candidates.append(hook_name.strip().strip("'\""))
    candidates += [
        "conky_hud_post", "conky_hud_pre", "conky_main_draw", "conky_main",
        "main_draw", "conky_draw",
    ]
    ...
    for name in candidates:
        # Greedy to end of file: correct when this function is last
        pat = re.compile(
        ...
        if not m:
            # Fallback: find function start, then take everything after it
            # up to the final standalone `end` in the file (still safer than
            # first nested end).
            m2 = re.search(
                rf"function\s+{re.escape(name)}\s*\([^)]*\)\s*\n",
                lua_text,
            ...
            if not m2:
                continue
            rest = lua_text[m2.end():]
            # Strip a trailing top-level end
            rest = re.sub(r"\nend\s*$", "", rest.strip())
            body = rest
            preamble = lua_text[: m2.start()].rstrip()
        else:
            body = m.group(1)
            preamble = lua_text[: m.start()].rstrip()

        # Drop a trailing `function conky_...` wrapper leftovers if any
        combined = (preamble + "\n\n" + body).strip() if preamble else body.strip()
        # Remove require('cairo') and surface lifecycle — Studio provides cr/W/H
        return _strip_known_boilerplate(combined)

    # Last resort: whole file as Custom Lua (still a valid escape hatch)
    return _strip_known_boilerplate(lua_text)


# ---------------------------------------------------------------------------
# Custom Lua ↔ Studio source bridge
# ---------------------------------------------------------------------------
# Pure-Lua themes (Batman, Skyrim, …) read vitals inside the draw hook.
# After import that becomes one Custom Lua node. Without a bridge, wiring
# a CPU source in the graph does nothing — the Lua never looks at SRC[].
#
# This bridge:
#   1. Detects common conky_parse / cache-key patterns in the Lua body
#   2. Ensures matching Studio source nodes exist
#   3. Wires them to Custom Lua in1..in12
#   4. Patches the Lua so bound inN values override the legacy read
#
# Unwired inputs are injected as nil by lua_gen (not 0), so
#   tonumber(in1) or safe_number('${cpu}', 0)
# falls through correctly when the user disconnects a wire.

# (detect_substrs, source_type, source_props, rewrite_patterns)
# rewrite_patterns: list of (regex, replacement using {in_slot} e.g. in1)
_LUA_BRIDGE_SPECS: list[tuple] = [
    (
        ["${cpu}", "safe_number('${cpu}'", 'safe_number("${cpu}"', "state.cpu"],
        "source.cpu_percent",
        {},
        [
            (r"state\.cpu\s*=\s*safe_number\(\s*['\"]\$\{cpu\}['\"]\s*,\s*state\.cpu\s*\)",
             "state.cpu = (tonumber({in_slot}) or safe_number('${{cpu}}', state.cpu))"),
            (r"safe_number\(\s*['\"]\$\{cpu\}['\"]\s*,\s*([^)]+)\)",
             "(tonumber({in_slot}) or safe_number('${{cpu}}', \\1))"),
        ],
    ),
    (
        ["${memperc}", "state.mem"],
        ...
        {},
        [
            (r"state\.mem\s*=\s*safe_number\(\s*['\"]\$\{memperc\}['\"]\s*,\s*state\.mem\s*\)",
             "state.mem = (tonumber({in_slot}) or safe_number('${{memperc}}', state.mem))"),
            (r"safe_number\(\s*['\"]\$\{memperc\}['\"]\s*,\s*([^)]+)\)",
             "(tonumber({in_slot}) or safe_number('${{memperc}}', \\1))"),
        ],
    ),
    (
        ["${fs_used_perc", "state.disk"],
        "source.disk_percent",
        {},
        [
            (r"state\.disk\s*=\s*safe_number\(\s*['\"]\$\{fs_used_perc[^'\"]*\}['\"]\s*,\s*state\.disk\s*\)",
             "state.disk = (tonumber({in_slot}) or safe_number('${{fs_used_perc /}}', state.disk))"),
        ],
    ),
    (
        ["cpu_temp", "sensors.cpu_temp", "SENSORS_CACHE"],
        "source.cpu_temp",
        {"poll_mode": "daemon", "poll_interval": 3},
        # Sensor-table overrides are applied in a second pass (see below)
        # so multiple keys can share one read_kv_cache site.
        [],
    ),
    (
        ["gpu_util", "sensors.gpu_util"],
        "source.gpu_util",
        {"poll_mode": "daemon", "poll_interval": 3},
        [],
    ),
    ...
    (
        ["disk_temp", "sensors.disk_temp"],
        "source.disk_temp",
        {"poll_mode": "daemon", "poll_interval": 30},
        [],
    ),
    (
        ["${downspeedf", "state.down_kbps"],
        "source.net_down",
        {},
        [
            (r"state\.down_kbps\s*=\s*safe_number\([^)]+\)",
             "state.down_kbps = (tonumber({in_slot}) or state.down_kbps)"),
        ],
    ),
    (
        ["${upspeedf", "state.up_kbps"],
        "source.net_up",
        {},
        [
            (r"state\.up_kbps\s*=\s*safe_number\([^)]+\)",
             "state.up_kbps = (tonumber({in_slot}) or state.up_kbps)"),
        ],
    ),
    (
        ["${uptime", "state.uptime"],
        ...
        {},
        [
            (r"state\.uptime\s*=\s*safe_parse\([^)]+\)",
             "state.uptime = (({in_slot} ~= nil and {in_slot} ~= '') and tostring({in_slot}) or state.uptime)"),
        ],
    ),
    (
        ...
        {},
        [
            (r"state\.hostname\s*=\s*safe_parse\([^)]+\)",
             "state.hostname = (({in_slot} ~= nil and {in_slot} ~= '') and tostring({in_slot}) or state.hostname)"),
        ],
    ),
    (
        ...
        {},
        [
            (r"state\.processes\s*=\s*safe_parse\([^)]+\)",
             "state.processes = (({in_slot} ~= nil) and tostring({in_slot}) or state.processes)"),
        ],
    ),
]


def _bridge_custom_lua_inputs(project: Project, ctx: "_ImportContext") -> int:
    """Create/wire sources into each Custom Lua node and patch the body.
    Returns number of edges created."""
    custom_luas = [n for n in project.nodes if n.type == "visual.custom_lua"]
    if not custom_luas:
        return 0

    edges_made = 0
    # Reuse one source node per type+props across multiple Custom Lua nodes
    source_by_key: dict[tuple, NodeInstance] = {}

    for lua_node in custom_luas:
        code = lua_node.props.get("code") or ""
        if not code.strip():
            continue
        next_slot = 1
        slot_map: list[tuple[int, str, list]] = []  # (slot, source_type, rewrite_pats)

        for detect_keys, src_type, src_props, rewrite_pats in _LUA_BRIDGE_SPECS:
            if next_slot > 12:
                break
            if not any(k in code for k in detect_keys):
                continue
            # Skip if this input slot already has an edge
            prop_key = f"in{next_slot}"
            if project.edge_for_prop(lua_node.id, prop_key) is not None:
                next_slot += 1
                continue

            key = (src_type, tuple(sorted((k, str(v)) for k, v in (src_props or {}).items())))
            src_node = source_by_key.get(key)
            if src_node is None:
                # Prefer an existing node of the same type already on the project
                for n in project.nodes:
                    if n.type != src_type:
                        continue
                    if all(str(n.props.get(k, "")) == str(v) for k, v in (src_props or {}).items()):
                        src_node = n
                        break
                if src_node is None:
                    try:
                        ...
                        next_slot += 1
                        continue
                    src_node = project.add_node(NodeInstance(
                        id=new_id("n"), type=src_type,
                        label=f"Bridge: {src_type.split('.')[-1]}",
                        props=dict(src_props or {}),
                    ))
                source_by_key[key] = src_node

            project.add_edge(src_node.id, lua_node.id, prop_key)
            edges_made += 1
            slot_map.append((next_slot, src_type, rewrite_pats))
            next_slot += 1

        if not slot_map:
            continue

        # Prepend helper + apply per-slot rewrites
        bridge_hdr = (
            "-- === Conky Studio input bridge (auto-wired on import) ===\n"
            "-- in1..in12 are injected by the generator (nil if unwired).\n"
            "-- When a slot is wired, the patches below prefer that value over\n"
            "-- the original conky_parse / cache read so the graph controls the HUD.\n"
            "local function _studio_num(bound, fallback)\n"
            "    local n = tonumber(bound)\n"
            "    if n ~= nil then return n end\n"
            "    return fallback\n"
            "end\n\n"
        )
        if "Conky Studio input bridge" not in code:
            code = bridge_hdr + code

        for slot, _stype, rewrite_pats in slot_map:
            in_name = f"in{slot}"
            for pat, repl_tmpl in rewrite_pats:
                repl = repl_tmpl.replace("{in_slot}", in_name)
                try:
                    code = re.sub(pat, repl, code, count=1)
                except re.error:
                    continue

        # Sensor-cache keys: inject overrides once after the first
        # state.sensors = read_kv_cache(...) so all bound slots apply.
        sensor_key_for_type = {
            "source.cpu_temp": "cpu_temp",
            "source.gpu_util": "gpu_util",
            "source.gpu_temp": "gpu_temp",
            "source.disk_temp": "disk_temp",
        }
        sensor_lines = []
        for slot, stype, _ in slot_map:
            key = sensor_key_for_type.get(stype)
            if not key:
                continue
            sensor_lines.append(
                f"    if tonumber(in{slot}) ~= nil then "
                f"state.sensors.{key} = tostring(in{slot}) end"
            )
        if sensor_lines:
            inject = "\n".join(sensor_lines)
            m = re.search(r"(state\.sensors\s*=\s*read_kv_cache\([^)]+\))", code)
            if m:
                code = code[: m.end()] + "\n" + inject + code[m.end():]
            else:
                # No state.sensors pattern — still document the slots
                code = (
                    "-- Sensor bridge (no state.sensors assignment found; "
                    "values available as inN):\n" + inject + "\n" + code
                )

        lua_node.props["code"] = code
        lua_node.props["bridge_slots"] = {
            f"in{slot}": stype for slot, stype, _ in slot_map
        }
...
# ---------------------------------------------------------------------------
# Import Custom -- hand-picked loose files (Lua / sh / conf / conkyrc),
# as opposed to import_legacy_theme's whole-theme-folder pipeline below.
# ---------------------------------------------------------------------------
# Deliberately narrow scope, same spirit as the module docstring above:
# this classifies and drops each file into the ONE node type it fits and
# stops there. It does not try to decompose hand-written Cairo into
# Studio's native visual nodes -- a Lua file with its own cairo_* calls
# becomes one Custom Lua node that keeps using its own drawing, forever.
# What it DOES do automatically:
#   - recognise a handful of known script basenames as native/family
#     sources instead of a generic Custom Script (same table as the
#     whole-theme importer)
#   - detect ${...}/cache-read patterns inside a Custom Lua node's body
#     and wire matching source nodes into its in1..in12 sockets (via the
#     existing _bridge_custom_lua_inputs bridge)
# Everything else is left for the user to hand-wire, or to just keep
# running as its own self-contained script/Lua -- both are fine outcomes.

# Extensions that are never imported: Python and anything else that
# doesn't run the way a Conky execi/daemon script or a Cairo Lua hook
# does. Skipped, never silently guessed at.
EXCLUDED_CUSTOM_EXTS = {
    ".py", ".pyc", ".pyw", ".pyo",
    ".rb", ".pl", ".php", ".js", ".ts", ".go", ".rs",
    ".exe", ".dll", ".so", ".jar", ".class", ".appimage",
    ".zip", ".tar", ".gz", ".7z", ".rar",
...
_SHELL_LIKE_EXTS = {".sh", ".bash", ".zsh", ".ksh"}


def _looks_like_shebang_script(path: str) -> bool:
    try:
        with open(path, "rb") as fh:
            head = fh.read(64)
    except OSError:
...
def classify_custom_file(path: str) -> str:
    """One of 'lua', 'conf', 'script', or 'skip'. Never raises."""
    base = os.path.basename(path)
    ext = os.path.splitext(base)[1].lower()

    if ext == ".lua":
        return "lua"
    if ext == ".conf" or base in ("conkyrc", ".conkyrc") or base.endswith("conkyrc"):
        return "conf"
    if ext in EXCLUDED_CUSTOM_EXTS:
        return "skip"
    if ext in _SHELL_LIKE_EXTS:
        return "script"
    if ext == "":
        return "script" if _looks_like_shebang_script(path) else "skip"
    # Unrecognised extension: still allow it in if it's clearly a shell
    # script wearing an odd name (e.g. a script named "sensors" with no
    # extension gets caught above; this catches e.g. "sensors.sh.bak"-ish
    # edge cases with a real shebang).
    return "script" if _looks_like_shebang_script(path) else "skip"


def import_custom_files(paths: list[str], project: "Project | None" = None) -> ImportResult:
    """Import a hand-picked set of loose files into *project* (a fresh one
    if not given). Accepts files or folders (folders are walked); each
    file is classified independently via classify_custom_file.

    - .conf / conkyrc: the FIRST one found gets the full whole-theme
      treatment (canvas settings + conky.text -> visual nodes), scoped to
      its own folder. Additional conf files are noted and skipped rather
      than merged -- import those separately.
    - .lua with cairo_*/cairo. calls: wrapped whole into one Custom Lua
      node, exactly like the whole-theme importer's draw-hook handling --
      it keeps using its own visuals, nothing is decomposed.
    - .lua with no Cairo calls: not a draw hook, so it becomes a Custom
      Script node instead of a visual (Studio still needs a `lua`
      interpreter on the command line to actually run one -- see the
      warning attached to that node).
    - .sh/.bash/other shebang scripts: mapped to a known native/family
      source when the basename matches (greeting.sh, playerctl.sh, ...),
      otherwise a generic Custom Script node with its body read in for
      inline editing.
    - Python and other non-shell/Lua/Conky files: skipped with a warning,
      never guessed at.

    After nodes are created, any Custom Lua node's body is scanned for
    recognised ${...}/cache-read patterns and matching source nodes are
    wired into its in1..in12 sockets -- the same bridge the whole-theme
    importer uses. Nothing about this step is required for the imported
    nodes to work; it's a convenience so the graph *can* drive them.
    """
    expanded: list[str] = []
    for p in paths:
        if os.path.isdir(p):
            for root, dirs, files in os.walk(p):
                dirs[:] = [d for d in dirs if d not in {".git", "__pycache__", "_imported_scripts"}]
                for f in files:
    ...
    warnings: list = []
    buckets: dict[str, list[str]] = {"lua": [], "conf": [], "script": [], "skip": []}
    for p in expanded:
        buckets[classify_custom_file(p)].append(p)

    for p in buckets["skip"]:
        ext = os.path.splitext(p)[1].lower() or "(no extension)"
        warnings.append(
            f"Skipped {os.path.basename(p)} -- {ext} isn't something Studio can wire up as a "
            f"Conky source or Cairo visual (Python and other non-shell/Lua files are excluded)."
        )

    conf_paths, lua_paths, script_paths = buckets["conf"], buckets["lua"], buckets["script"]

    if project is None:
        if conf_paths:
            primary = conf_paths[0]
            result = import_legacy_theme(os.path.dirname(primary) or ".", os.path.basename(primary))
            project = result.project
            warnings = result.warnings + warnings
            # Don't double-import files the whole-theme pass already pulled in.
            already_lua = {n.props.get("source_file") for n in project.nodes if n.type == "visual.custom_lua"}
            lua_paths = [p for p in lua_paths if os.path.basename(p) not in already_lua]
            already_scripts = {n.props.get("script_basename") for n in project.nodes if n.type == "source.custom_script"}
            script_paths = [p for p in script_paths if os.path.basename(p) not in already_scripts]
            conf_paths = conf_paths[1:]
        else:
            project = Project(name="Custom import", description="Imported via Conky Studio's Import Custom.")
            project.ensure_canvas_node()
    else:
        # Merging into an existing project: fully parsing another conf's
        # canvas/TEXT into it would clobber settings, so treat every conf
        # here as "extra" (see the loop below).
        pass

    for extra_conf in conf_paths:
        warnings.append(
            f"Only one .conf/conkyrc gets the full canvas + TEXT treatment per import; "
            f"{os.path.basename(extra_conf)} was skipped -- import it on its own if you need it too."
        )

    anchor_dir = os.path.dirname((lua_paths or script_paths or [None])[0] or "") or "."
    ctx = _ImportContext(project, anchor_dir, os.path.join(anchor_dir, "_imported_scripts"))

    for lua_path in lua_paths:
        lua_text = _read(lua_path)
        base = os.path.basename(lua_path)
        if "cairo_" in lua_text or "cairo." in lua_text:
            body = _extract_draw_hook_body(lua_text, None)
            project.add_node(NodeInstance(
                id=new_id("n"), type="visual.custom_lua", z=project.next_z(),
                label=f"Imported Lua ({base})",
                props={
                    "code": body, "x": 0, "y": 0,
                    "standalone_import": True, "source_file": base,
                },
            ))
            warnings.append(
                f"{base} has its own Cairo drawing -- imported whole as a Custom Lua node using "
                f"that drawing as-is. Studio doesn't decompose hand-written Cairo into native "
                f"visual nodes; edit the Lua text directly to change how it looks."
            )
            mouse_count = parse_mouse_regions(lua_text, project)
            if mouse_count:
                warnings.append(f"Extracted {mouse_count} click region(s) from {base}.")
        else:
            project.add_node(NodeInstance(
                id=new_id("n"), type="source.custom_script",
                label=f"Script: {base}",
                props={
                    ...
                    "script_body": _read_script_body_for_edit(lua_path),
                    "poll_mode": "execi", "poll_interval": 5, "output_kind": "text",
                    "script_basename": base,
                },
            ))
            warnings.append(
                f"{base} has no Cairo drawing calls, so it went into a Custom Script node "
                f"instead of a visual -- wire its output to a visual by hand if that's the "
                f"intent. Studio needs a `lua` interpreter available on the command line to "
                f"actually run it (e.g. set the script path to run `lua5.3 {base}`)."
            )

    for script_path in script_paths:
        base = os.path.basename(script_path)
        if base.lower() in ("start.sh", "stop.sh", "install.sh"):
            warnings.append(f"Skipped {base} -- looks like a theme launcher; Studio generates its own.")
            continue
        kind, mapped_type, extra = _classify_known_script(base)
        if kind in ("native", "family") and mapped_type:
            if not any(n.type == mapped_type for n in project.nodes):
                props = {"poll_mode": "execi", "poll_interval": 5} if kind == "family" else {}
                project.add_node(NodeInstance(id=new_id("n"), type=mapped_type, props=props))
            warnings.append(f"Recognised {base} -- mapped to its matching built-in source node.")
            continue
        project.add_node(NodeInstance(
            id=new_id("n"), type="source.custom_script",
            label=f"Script: {base}",
            props={
                "script_path": script_path,
                "script_body": _read_script_body_for_edit(script_path),
                "poll_mode": "execi", "poll_interval": 5,
                "output_kind": (extra or {}).get("output_kind", "text"),
                "script_basename": base,
            },
        ))

    edges = _bridge_custom_lua_inputs(project, ctx)
    if edges:
        warnings.append(
            f"Auto-wired {edges} matching source(s) into imported Custom Lua input slot(s) "
            f"(in1..in12) based on recognised ${{...}}/cache reads in the code."
        )

    _layout_import_graph(project)
    return ImportResult(project=project, warnings=_collapse_warnings(warnings))
...
def import_legacy_theme(theme_dir: str, conf_filename: str = None) -> ImportResult:
    ...
    for lua_path in lua_paths:
        ...
        if "cairo_" in lua_text or "cairo." in lua_text:
            ...
            # This is an escape-hatch node: all HUD logic stays inside this text.
            # Companion shell scripts are separate Custom Script nodes (daemon).
            project.add_node(NodeInstance(
                id=new_id("n"), type="visual.custom_lua", z=project.next_z(),
                ...
                props={
                    ...
                    "source_file": os.path.basename(lua_path),
                },
            ))
            ctx.warnings.append(
                f"{os.path.basename(lua_path)} imported as a self-contained Custom Lua node "
                f"(helpers + {hook_name or 'draw-hook'} body; surface setup stripped for Studio's "
                f"cr/W/H). Its internal logic is unchanged — edit the Lua text to change behavior. "
                f"Companion scripts are separate Custom Script nodes."
            )
    ...
    for script_path in ordered:
        ...
        # keeps them running; builder + start_sh_gen include unwired
        # daemon custom scripts, and self-caching ones keep their original
        # basename so CACHE_FILE names (sensors.cache) stay correct.
        ...
        self_caching = _is_self_caching_script(body) or _is_self_caching_script(
            _read(script_path) if os.path.isfile(script_path) else ""
        )
        # Heuristic: known names are always treated as self-caching.
        if base.lower() in ("sensors.sh", "weather.sh", "weather-text-icon", "forecast-text-icon"):
            self_caching = True
        conf_path = _find_companion_conf(script_path)
        props = {
            "script_path": patched,
            "script_body": body,
            "poll_mode": "daemon",
        ...
        if self_caching:
            props["self_caching"] = True
        if conf_path:
            props["companion_conf"] = conf_path
            ctx.warnings.append(
                f"Found companion config {os.path.basename(conf_path)} for {base} "
                f"-- it will be copied next to the script at build time."
            )
        project.add_node(NodeInstance(
            id=new_id("n"), type="source.custom_script",
            label=f"Script: {base}",
    ...
    # Pure-Lua themes draw images *inside* the Custom Lua body. We:
    #   1. Rewrite absolute paths in the Lua to ASSETS_DIR / CACHE_DIR
    #   2. Attach asset_paths so builder copies files into assets/ + images/
    #   3. Bridge common sensor reads to Custom Lua in1..in12 (below) so
    #      Studio sources can be rewired without hand-editing the Lua.
    ...
    if theme_images:
        ctx.warnings.append(
            ...
            f"with the Custom Lua node (Build copies them into assets/ and images/) and "
            f"paths inside the Lua were rewritten to ASSETS_DIR / CACHE_DIR. "
            f"Add a separate Image/Icon if you want Studio rotation/glow as an extra layer."
        )
    if total_rewrites:
        ctx.warnings.append(
            f"Rewrote {total_rewrites} absolute/HOME-relative path(s) inside "
            f"Custom Lua to use ASSETS_DIR / CACHE_DIR."
        )

    # Custom Lua + Custom Script are independent escape hatches: their logic
    # stays inside their own text (no forced Studio source wiring / rewrites).
    # Graph nodes you add later are separate layers. To mix them, edit the
    # Custom Lua/Script text to read in1..in12 or call other scripts yourself.
    ctx.warnings.append(
        "Import model: Custom Lua and companion scripts are self-contained escape "
        "hatches (their own CPU/GPU/weather logic). Studio does not rewire that "
        "logic into graph sources. Daemon scripts still run via start.sh so caches "
        "update. To use graph data inside Custom Lua, wire Inputs 1–12 and edit "
        "the Lua to read in1..in12."
    )
